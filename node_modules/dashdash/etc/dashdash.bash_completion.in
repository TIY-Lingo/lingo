    ...
    function _dashdash_complete {
        ...
        arg)
            ...
            if [[ "${word:0:1}" == '$' ]]; then
                ...
                echo ""
            elif ! section complete_$argtype 2>/dev/null >/dev/null; then
...
if section complete &>/dev/null; then
...
elif section compdef &>/dev/null; then
    function _{{name}}_completion {
        local _log_file=/dev/null
        [[ -z "$_{{name}}_log" ]] || _log_file="$_{{name}}_log"
...
elif section compctl &>/dev/null; then
    function _{{name}}_completion {
