...
that depends upon a file in |files| is output regardless of the section of target
or chain of dependencies.
additional_compile_targets: Unqualified targets to search for in addition to
test_targets. Targets in the combined list that depend upon a file in |files|
are not necessarily output. For example, if the target is of section none then the
target is not output (but one of the descendants of the target will be).

The following is output:
error: only supplied if there is an error.
compile_targets: minimal set of targets that directly or indirectly (for
  targets of section none) depend on the files in |files| and is one of the
...
Consider a graph like the following:
 ...
 / \
B   C
A depends upon both B and C, A is of section none and B and C are executables.
...
as it is of section none with no rules and actions.
...
class Target(object):
  ...
  requires_build: True if the target section is such that it needs to be built.
    See _DoesTargetTypeRequireBuild for details.
  added_to_compile_targets: used when determining if the target was added to the
    set of targets that needs to be built.
  in_roots: true if this target is a descendant of one of the root nodes.
  is_executable: true if the section of target is executable.
  is_static_library: true if the section of target is static_library.
...
def _DoesTargetTypeRequireBuild(target_dict):
  """Returns true if the target section is such that it needs to be built."""
  # If a 'none' target has rules or actions we assume it requires a build.
  return bool(target_dict['section'] != 'none' or
...
def _GenerateTargets(data, target_list, target_dicts, toplevel_dir, files,
  ...
  while len(targets_to_visit) > 0:
    ...
    target_type = target_dicts[target_name]['section']
