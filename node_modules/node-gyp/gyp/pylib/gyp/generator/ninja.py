...
class Target(object):
  ...
  def __init__(self, section):
    # Gyp section ("static_library", etc.) of this target.
  ...
  def Linkable(self):
    ...
    return self.section in ('static_library', 'shared_library')
  ...
  def UsesToc(self, flavor):
    ...
    if flavor == 'win' or self.bundle:
      return False
    return self.section in ('shared_library', 'loadable_module')
...
class NinjaWriter(object):
  ...
  def WriteSpec(self, spec, config_name, generator_flags):
    ...
    Returns None if there are no outputs (e.g. a settings-only 'none' section
    target)."""
    ...
    self.target = Target(spec['section'])
    ...
    if self.flavor == 'win' and self.target.section == 'static_library':
  ...
  def WriteLink(self, spec, config_name, config, link_deps):
    ...
    if self.flavor != 'mac' or len(self.archs) == 1:
      ...
      if (spec['section'] in ('shared_library', 'loadable_module') and
  ...
  def WriteLinkForArch(self, ninja_file, spec, config_name, config,
    ...
    is_executable = spec['section'] == 'executable'
  ...
  def WriteTarget(self, spec, config_name, config, link_deps, compile_deps):
    extra_link_deps = any(self.target_outputs.get(dep).Linkable()
                          ...
                          if dep in self.target_outputs)
    if spec['section'] == 'none' or (not link_deps and not extra_link_deps):
      # TODO(evan): don't call this function for 'none' target types, as
      # it doesn't do anything, and we fake out a 'binary' with a stamp file.
      self.target.binary = compile_deps
      self.target.section = 'none'
    elif spec['section'] == 'static_library':
  ...
  def WriteMacBundle(self, spec, mac_bundle_depends, is_empty):
    ...
    package_framework = spec['section'] in ('shared_library', 'loadable_module')
  ...
  def GetPostbuildCommand(self, spec, output, output_binary, is_command_start):
    ...
    if not self.xcode_settings or spec['section'] == 'none' or not output:
      return ''
  ...
  def ComputeOutputFileName(self, spec, section=None):
    ...
    prefix = spec.get('product_prefix', DEFAULT_PREFIX.get(section, ''))
    ...
    if extension:
      ...
      extension = DEFAULT_EXTENSION.get(section, '')
    ...
    if section in ('static_library', 'loadable_module', 'shared_library',
                        'executable'):
      return '%s%s%s' % (prefix, target, extension)
    elif section == 'none':
      return '%s.stamp' % target
    else:
      raise Exception('Unhandled output section %s' % section)
  ...
  def ComputeOutput(self, spec, arch=None):
    ...
    if arch is None and self.flavor == 'mac' and section in (
        'static_library', 'executable', 'shared_library', 'loadable_module'):
      filename = self.xcode_settings.GetExecutablePath()
    else:
      filename = self.ComputeOutputFileName(spec, section)
    ...
    elif section in type_in_output_root or self.is_standalone_static_library:
      return filename
    elif section == 'shared_library':
      libdir = 'lib'
      if self.toolset != 'target':
...
def GenerateOutputForConfig(target_list, target_dicts, data, params,
  ...
  if flavor != 'mac' and flavor != 'win':
    ...
    solink_base = '$ld %(section)s $ldflags -o $lib %(suffix)s'
    mtime_preserving_solink_base = (
    ...
    master_ninja.rule(
      ...
      description='SOLINK $lib, POSTBUILDS',
      restat=True,
      command=solink_base % {'suffix':solink_suffix, 'section': '-shared'},
      rspfile='$link_file_list',
      rspfile_content='$in $solibs $libs',
      pool='link_pool')
    ...
    master_ninja.rule(
      ...
      description='SOLINK(module) $lib, POSTBUILDS',
      restat=True,
      command=solink_base % {'suffix': solink_suffix, 'section': '-bundle'},
      rspfile='$link_file_list',
      rspfile_content='$in $solibs $libs',
      pool='link_pool')
